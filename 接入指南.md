...
TVS-Lite API是TVS API精简版， TVS API需要终端实现复杂的HTTP协议，建立下行通道、维持下行通道等。TVS-Lite API仅需要实现简单的请求与应答。
...
基本directive、event、交互流程与TVS API一致。 见[TVS API指令与事件](https://github.com/TencentDingdang/tvs-tools/blob/master/doc/AVS兼容API接口文档.md)
